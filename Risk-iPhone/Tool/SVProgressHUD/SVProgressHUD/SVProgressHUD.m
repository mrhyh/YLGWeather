...
+ (SVProgressHUD*)sharedView {
    ...
    if (duration == UIDeviceOrientationLandscapeLeft) {
        sharedView.transform = CGAffineTransformMakeRotation((90 * M_PI) / 180.0f);
    }else {
        sharedView.transform = CGAffineTransformMakeRotation((0 * M_PI) / 180.0f);
    }
    ...
    dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:frame];
        
        
    });
#else
    
    CGRect twoFrame = [[UIScreen mainScreen] bounds];
    twoFrame.origin.y = 64;
    twoFrame.size.height = frame.size.height-64-49;
    
    dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame: ]; });
#endif
    sharedView.backgroundColor = [UIColor redColor];
    return sharedView;
}
...
#warning TODO 测试
        ...
        _backgroundLayerColor = [UIColor redColor];
    }
