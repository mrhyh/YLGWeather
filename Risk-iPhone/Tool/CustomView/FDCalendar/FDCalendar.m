...
@property (nonatomic, copy) FDCalendarCompleteBlock fdCalendarCompleteBlock;
@end
...
- (instancetype)initWithFrame:(CGRect)frame  currentDate:(NSDate *)date fdCalendarCompleteBlock:(FDCalendarCompleteBlock)block{
    ...
    if (self = [super init]) {
        ...
        [self setFrame:CGRectMake(frame.origin.x, frame.origin.y, _customFrame.size.width, CGRectGetMaxY(self.scrollView.frame))];
        
        [self setCurrentDate:self.date];
        
        self.fdCalendarCompleteBlock = block;
    }
    return self;
}
...
// 设置上层的titleBar
- (void)setupTitleBar {
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _customFrame.size.width, 44)];
    //titleView.backgroundColor = [UIColor redColor];
    titleView.backgroundColor = RSVisitArchiveButtonColor;
...
- (void)setupWeekHeader {
    ...
    for (int i = 0; i < count; i++) {
        UILabel *weekdayLabel = [[UILabel alloc] initWithFrame:CGRectMake(offsetX, 50, (_customFrame.size.width - 10) / count, 20)];
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(15, 74, _customFrame.size.width - 30, 1)];
...
- (void)setupScrollView {
    ...
    [self.scrollView setFrame:CGRectMake(0, 75, _customFrame.size.width, self.centerCalendarItem.frame.size.height)];
...
- (void)setupCalendarItems {
    ...
    self.leftCalendarItem = [[FDCalendarItem alloc] initWithFrame:_customFrame];
    [self.scrollView addSubview:self.leftCalendarItem];
    
    CGRect itemFrame = self.leftCalendarItem.frame;
    itemFrame.origin.x = _customFrame.size.width;
    self.centerCalendarItem = [[FDCalendarItem alloc] initWithFrame:_customFrame];
    ...
    itemFrame.origin.x = _customFrame.size.width * 2;
    self.rightCalendarItem = [[FDCalendarItem alloc] initWithFrame:_customFrame];
...
- (void)calendarItem:(FDCalendarItem *)item didSelectedDate:(NSDate *)date {
    ...
    if (self.fdCalendarCompleteBlock ) {
        _fdCalendarCompleteBlock(date);
    }
