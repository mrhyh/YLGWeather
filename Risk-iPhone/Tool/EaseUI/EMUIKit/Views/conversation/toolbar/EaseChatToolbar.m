...
{
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:20 type:EMChatToolbarTypeGroup]; //输入框最高的高度
    if (self) {
...
{
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:85 type:type];
    if (self) {
...
- (void)_setupSubviews
{
    ...
    CGFloat height = self.frame.size.height - self.verticalPadding * 2;
    _inputTextView.autoresizingMask = UIViewAutoresizingFlexibleHeight;
    _inputTextView.scrollEnabled = YES;
    _inputTextView.returnKeyType = UIReturnKeySend;
    _inputTextView.enablesReturnKeyAutomatically = YES; // UITextView内部判断send按钮是否可以用
    _inputTextView.placeHolder = NSEaseLocalizedString(@"message.toolBar.inputPlaceHolder", @"输入新消息");
    ...
    styleChangeButton.hidden = YES; //暂时隐藏语音输入
...
- (UIView *)faceView
{
    ...
    if (_faceView == nil) {
        ...
        CGRect tempRect = _faceView.frame;
