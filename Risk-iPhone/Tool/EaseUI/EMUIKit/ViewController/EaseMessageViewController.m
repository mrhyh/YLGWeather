...
@interface EaseMessageViewController () <TZImagePickerControllerDelegate,EaseMessageViewControllerDataSource, EaseMessageViewControllerDelegate>
{
...
#warning TODO 可能是环信默认加载的聊天记录条数
        _messageCountOfPage = 200;  //
        _timeCellHeight = 30;
...
- (void)viewDidLoad {
    ...
    self.chatToolbar.autoresizingMask = UIViewAutoresizingFlexibleTopMargin; //原来的
    //self.chatToolbar.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin;
    //初始化手势
...
- (void)dealloc
{
    ...
    if (_tzImagePickerController) {
        [_tzImagePickerController dismissViewControllerAnimated:NO completion:nil];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (YES == isRefersh) {
#warning TODO 测试 注销时清空tableView的数据，避免切换账户时自己曾经发送的消息被作为群消息发出来，似乎有大问题
...
- (TZImagePickerController *)tzImagePickerController {
    if (_tzImagePickerController == nil ) {
        _tzImagePickerController = [[TZImagePickerController alloc] init];
...
- (void)setChatToolbar:(EaseChatToolbar *)chatToolbar
{
    ...
    CGRect tableFrame2 = _chatToolbar.frame;
...
- (void)_scrollViewToBottom:(BOOL)animated
{
    if (self.tableView.contentSize.height > self.tableView.frame.size.height)
    {
        ...
        CGRect tempFrame1 = self.tableView.frame;
    }
}
...
- (void)_videoMessageCellSelected:(id<IMessageModel>)model
{
    ...
    dispatch_block_t block = ^{
        ...
        [self.view.window.rootViewController presentViewController:moviePlayerController animated:YES completion:nil];
    };
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    NSLog( @"数组大小:%lu", (unsigned long)[self.dataArray count]);
...
- (void)moreViewPhotoAction:(EaseChatBarMoreView *)moreView
{
    ...
    [self.view.window.rootViewController presentViewController:imagePickerVc animated:YES completion:nil];
}
...
#warning TODO 避免销毁此控制器然后再次进入时聊天记录消失，找到确切原因后再解决移除，不能删除此段代码。
            model = [_dataSource messageViewController:self modelForMessage:message];
        }
...
- (void)_sendMessage:(EMMessage *)message
{
    ...
    //
    //        if ([UIUtil isNoEmptyStr:aError.errorDescription] ) {
    //            NSLog(@"发送消息失败，原因:%@",aError.errorDescription);
    //            [UIUtil alert:@"发送失败，请重新登录账号再发送消息"];
    //        }
    //        [weakself.tableView reloadData];
    //    }];
    ...
    [[EMClient sharedClient].chatManager sendMessage:message progress:^(int progress) {
        ...
        if ([UIUtil isNoEmptyStr:error.errorDescription] ) {
            NSLog(@"发送消息失败，原因:%@",error.errorDescription);
            [UIUtil alert:@"发送失败，请重新登录账号再发送消息"];
        }
        [weakself.tableView reloadData];
    }];
