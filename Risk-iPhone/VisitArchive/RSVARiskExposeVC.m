...
@interface RSVARiskExposeVC ( ) <UITableViewDelegate, UITableViewDataSource, RSCalendarVCDelegate, UITextFieldDelegate, RSVADeleteVCDelegate, RSVADeleteVCDelegate,UIGestureRecognizerDelegate>
...
@implementation RSVARiskExposeVC {
...
- (void) initData {
    ...
    nameTextFieldW = 100;
    nameToTextFieldSpace= 7.5;
    textFieldSpaceToName = 10;
...
- (void) createLeftHeadView: (UIView *)view{
    ...
    _riskFactorTextField = [self createTextFieldWith:@"点击选择风险因素"];
    _dosageTextField = [self createTextFieldWith:@"剂量"];
    _dosageTextField.keyboardType = UIKeyboardTypeNumbersAndPunctuation;
    _exposureTimeStartDataTextField = [self createTextFieldWithImageName:nil text:@"开始日期"];
    _exposureTimeEndDataTextField = [self createTextFieldWithImageName:nil text:@"结束日期"];
    ...
    .rightSpaceToView(_dosageTextField, 5)
    .topEqualToView(staticLTVTwoLabel)
    .widthIs(45)
    .heightIs(buttonH);
    ...
    .leftSpaceToView(staticLTVTwoLabel, spaceToLeft)
    ...
    .leftSpaceToView(staticLTVFourLabel, spaceToLeft)
    .rightSpaceToView(contentView, spaceToLeft)
    ...
    .topSpaceToView(staticLTVFourLabel,spaceToTopView)
    .leftEqualToView(_exposureTimeStartDataTextField)
    .rightSpaceToView(contentView, spaceToLeft)
    .heightIs(buttonH);
...
//    .heightIs(buttonH);
    ...
    _medicalRecordNumLabel.text = @"暴露因素名称";
    _nameLabel.text = @"剂量";
    _phoneNumLabel.text = @"暴露时间范围";
}
...
- (void ) navigationRightAction {
    if (self.leftDataSource.count > 0)  { //有数据
        self.rsVAPatientArchiveVCPatientrisksModelBlock(self.leftDataSource);
        [self.navigationController popViewControllerAnimated:YES];
    }else {
        [self riskAddButtonAction];
    }
}
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (_leftHeadView == nil ) {
        _leftHeadView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 144)];
        _leftHeadView.backgroundColor = RSVisitArchiveTableViewHeadSectionBGColor;
